	...
	if hit_area.monitoring:
		...
		for body in bodies:
			if body.is_in_group("Player"):
				...
				if get_node(GlobalPaths.PLAYER).dashing: return
				...
				if in_jump_boots and (body.was_falling or not body.is_on_floor()): return

				if cooling_down: return

				_hit_area_body_entered(body)
				#print(get_parent().get_parent().name + " Damage Player from invincibility")
