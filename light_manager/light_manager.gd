...
#	size *= get_node(Globals.player_camera_path).zoom
...
func set_pos():
	var pos_x = stepify(get_node(Globals.player_body_path).position.x, snap_step) - size.x / 2
	var pos_y = stepify(get_node(Globals.player_body_path).position.y, snap_step) - size.y / 2
